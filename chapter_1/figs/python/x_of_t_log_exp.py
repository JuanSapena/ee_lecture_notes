...
x = np.arange(T)
plt.semilogy(x, np.exp(x*(.6+1.5-2)/2), 'lightblue', label='Expectation value', linewidth=4)
plt.semilogy(x, ensemble.iloc[45,:], 'b-', label='$N=1$')
plt.semilogy(x, np.mean(ensemble.iloc[0:100,:]), 'g-', label='$N=100$')
plt.semilogy(x, np.mean(ensemble.iloc[0:10000,:]), 'r-', label='$N=10,000$')
plt.semilogy(x, np.mean(ensemble), 'k-', label='$N=1,000,000$')
plt.legend()
plt.xlabel('$t$')
plt.ylabel('$x(t)$')


plt.savefig("./../x_of_t_log_exp.pdf", bbox_inches='tight')
plt.show()
