...
We therefore try a different analysis. \Fref{1_3} shows another simulation of your wealth, 
...
This is $\ave{\D \x}$, according to \eref{exp_sum} 
...
\ee
We denote by $\rt$ the effective factor by which $\x(\t)$ is multiplied per round when the change is computed over a long time. This quantity is found by taking the $\Dt^{\text{th}}$ root of $\frac{\x(\t+\Dt)}{\x(\t)}$ and considering the long-time limit
\bea
\rt &=&\lim_{\Dt\to\infty }\left(\frac{\x(\t+\Dt)}{\x(\t)}\right)^{1/\Dt}\\
...
$\Dt=\T \dt$
\be
\gm(\ave{\x}_\N, \Dt)=\frac{1}{\gdelta \T}  \sum_{\gtau=1}^{\T} \D\ln \ave{\x}_\N(\gtau).
\ee
...
\be
\Abar_{\Dt}=\frac{1}{\T}\sum_{\gtau=1}^{\T}\A(\gtau \dt).
\elabel{t_ave_f_disc}
\ee
If it changes continuously the finite-time average is
\be
\Abar_{\Dt}=\frac{1}{\Dt}\int_0^{\Dt} \A(\gs)\gd\gs.
\elabel{t_ave_f}
\ee
\end{defn}
...
\be
\lim_{\Dt \to\infty}\frac{1}{\Dt } \int_0^{\Dt} \A(\gs) \gd\gs =\lim_{\N\to\infty} \frac{1}{\N}\sum_\gi^\N \A_\gi .
\elabel{def_ergodic}
\ee
\end{defn}
...
to find the monotonic function of $\x$ that inherits the stationarity of the ink in \eref{law}.
The game is defined by a set of factors of increase in $\x(\t)$, \eref{gamble}. Therefore, the fractional change in $\x$, namely
