...
\be
\ave{\A(\t)}_{\N}=\frac{1}{\N}\sum_{\gi}^{\N} \A_{\gi}(\t),
\elabel{f_ens}
\ee 
...
\end{itemize}
That's it. The set may be discrete, like $\{4, 7.8, 29\}$, or it may be continuous, like the real line or the positive reals. The \PDFa is a non-negative function of the set
\be
\PDF(x)\geq 0.
\ee
It is normalized, so that the integral over the entire set is one,
\be
\int \PDF(x)dx =1.
\ee
Note the standard use of capital and small letters: $X$ is the random variable and $x$ is a value it may take.
The probability (a number between 0 and 1) that $x$ is between $a$ and $b$ is
\be
\int_a^b \PDF(x)dx.
\ee
When the set of possible values is discrete, we will write $p_i$ to mean the probability of the $i^{\text{th}}$ value in the set, $x_i$. We could also express this as an integral of the \PDFa over a neighborhood of $x_i$ that includes no other possible values.

Of course we can define the distribution of a random variable to depend on all kinds of things -- the day of the week, or the country we find ourselves in. Sometimes it will be useful to consider distributions of random numbers that depend on time. By default we will assume that the distributions of random numbers are time-independent, but to be sure we will often make this explicit.
\end{excursion}
...
\bea
\ave{\x(\t)}_\N&=&\frac{1}{\N}\sum_\gi  \x_\gi(\t)\\
...
Once again, we recommend van Kampen \cite[p.~52]{vanKampen1992} for an exceedingly simple definition -- this time of a stochastic process. 
Imagine we've defined a random variable, $X$. Any function $Y(X)$ is then also a random variable. A stochastic process is a special case of such a function, namely one that depends on an additional variable $\t$, which is interpreted as time, so we write
\be
Y_X(\t)=f(X,\t).
\elabel{st_pr}
\ee
This may not be how you think of a stochastic process. When we simulate stochastic processes, we often start with some value and modify it in some way in each step of a for-loop. In each step of the for-loop we would generate a new instance of a random number and thereby construct the trajectory of the stochastic process. In \eref{st_pr} it's not generated that way. Instead, in this picture, we generate an instance $x$ of the random variable $X$ only once and insert that into \eref{st_pr}. The value $x$ specifies a simple function of time
\be
Y_x(\t)=f(x,\t)
\ee
meaning that all the randomness is contained in $x$. Once $x$ is specified, $Y_x(\t)$ is specified for all time, and we call it a ``realization'' or ``trajectory'' of the stochastic process. As an example you can think of drawing  at random a single uniformally distributed real number from the interval $(0,1)$. With probability 1, this number will be irrational and correspond to an infinite sequence of random decimal digits, which could be interpreted as a stochastic process, if $\t$ specifies the decimal place of the relevant digit.

We can also do this: fix a specific time, $\t^*$, and consider the stochastic process at that time, $Y_X(\t^*)$. That's again a random variable.
\end{excursion}
...
call this $\gn_\gj$. If $\gD$ is the amount of money in the pot, then 
we split the pot fairly by giving each player
 $\frac{\gn_\gj}{\N}\times \gD$.
This is $\ave{\gD}$, according to \eref{exp_sum}, 
...
We have two averages, $\rex$ and $\rt$ that we have determined numerically and analytically. 
...
The ``finite-time average'' of the observable $\A(\t)$ is
\be
\Abar_{\Dt}=\frac{1}{\Dt}\int_{\t}^{\t+\Dt} \A(\gs)\gd\gs.
\elabel{t_ave_f}
\ee
If $\A$ only changes at $\T=\Dt/\dt$ discrete times 
$\t+\dt$, $\t+2\dt$, {\it etc.}, then this can be written as 
\be
\Abar_{\Dt}=\frac{1}{\T\dt}\sum_{\gtau=1}^{\T}\A(\t+\gtau \dt).
\elabel{t_ave_f_disc}
\ee
\end{defn}
...
In terms of random variables, $X$, and stochastic processes, $Y_X(t)$, the ergodic property can be
...
\begin{picture}(200,220)(0,0)
  \put(-5,-20){\includegraphics[width=\textwidth]{./chapter_1/figs/ergodic_grid.pdf}}
  \put(10,195){$Y_{x_1}(\t)$}
  \put(10,155){$Y_{x_2}(\t)$}
  \linethickness{.5mm}
  \put(140,212){$\t^*$}  
  \put(160,195){$Y_{x_1}(\t)$}
  \put(160,209){\vector(-1,-1){15}}
  \put(10,115){$Y_{x_3}(\t)$}
  \put(10,75){$Y_{x_4}(\t)$}
  \put(10,35){$Y_{x_5}(\t)$}
\end{picture}
\caption{Time runs from left to right in this figure, and realizations run from top to bottom.
Averaging over time means averaging along one trajectory from left to right; averaging over the ensemble
...
%\psset{unit=1pt}
%\begin{pspicture}(0,0)(200,220)
%  \put(-5,-20){\includegraphics[width=\textwidth]{./chapter_1/figs/ergodic_grid.pdf}}
%    \put(10,195){$Y_{x_1}(\t)$}
%  \put(10,155){$Y_{x_2}(\t)$}
%  \linethickness{.5mm}
%  \put(40,155){\vector(1,0){300}}
%  \put(40,155){\includegraphics[width=\textwidth]{./chapter_1/figs/arrow1.pdf}}
% \put(140,212){$\t^*$}  
% \put(140,210){\vector(0,-1){200}}
%  \put(10,115){$Y_{x_3}(\t)$}
%  \put(10,75){$Y_{x_4}(\t)$}
%  \put(10,35){$Y_{x_5}(\t)$}
%\end{pspicture}
...
\end{itemize}
These and other properties of the log-normal distribution will be discussed in detail in \secref{populations}.
