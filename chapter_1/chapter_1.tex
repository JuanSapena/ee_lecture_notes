...
\ee
We denote by $\rt$ the effective factor by which $\x(\t)$ is multiplied per round when the change is computed over a long time. This quantity is found by taking the $\t^{\text{th}}$ root of $\frac{\x(\t+\Dt)}{\x(\t)}$ and considering the long-time limit
\bea
