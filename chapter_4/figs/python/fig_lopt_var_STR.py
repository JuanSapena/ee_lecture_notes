...
lopt_var_STR=pd.read_pickle(input_dir+"STR-FET_lopt_var.pkl")
lopt_var_BTC=pd.read_pickle(input_dir+"BTC-FET_lopt_var.pkl")
lopt_var_DAX=pd.read_pickle(input_dir+"DAX-BNT_lopt_var.pkl")
...
#
#returns_STR=pd.read_pickle(input_dir+data_ID+"-1.pkl")
#equity_STR=np.cumprod(returns_1)
#final_equity_STR=equity_1[-1:].T
##
...
years_STR=parameters.loc['years','STR']
sigma_STR=parameters.loc['sigma_est','STR']
years_DAX=parameters.loc['years','DAX']
sigma_DAX=parameters.loc['sigma_est','DAX']
years_BTC=parameters.loc['years','BTC']
sigma_BTC=parameters.loc['sigma_est','BTC']
...
#STR
plt.loglog(lopt_var_STR.index.days,(lopt_var_STR.index.days/365.25)**(-0.5)/sigma_STR,linestyle='--',color='blue')
plt.loglog(lopt_var_STR.index.days,np.sqrt(lopt_var_STR),color='blue',marker='o',linestyle='')
#DAX
plt.loglog(lopt_var_DAX.index.days,(lopt_var_DAX.index.days/365.25)**(-0.5)/sigma_DAX,linestyle='--',color='orange')
plt.loglog(lopt_var_DAX.index.days,np.sqrt(lopt_var_DAX),color='orange',marker='o',linestyle='')
#BTC
plt.loglog(lopt_var_BTC.index.days,(lopt_var_BTC.index.days/365.25)**(-0.5)/sigma_BTC,linestyle='--',color='red')
plt.loglog(lopt_var_BTC.index.days,np.sqrt(lopt_var_BTC),color='red',marker='o',linestyle='')
