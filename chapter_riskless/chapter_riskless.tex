...
it's not time-translation invariant for additive growth, \eref{linx}. Substituting \eref{linx} in 
\eref{exp_rate} gives
\bea
\gexp&=& \frac{\ln \xd(\t+\Dt)-\ln \xd(\t)}{\Dt}\\
...
but (see \RHS of \eref{exp_lin}) depends on $\xd(\t)$, \ie on the time when we started 
...
But the exponential growth rate is commonly used, and for good reasons. Let's see what it's 
...
We require that \eref{exp_rate} yield a constant, let's call that $\ggamma$ again, irrespective 
...
But there's a simpler way of finding the transformation $\gv(\xd)$ that doesn't involve integrals and differential 
...
$\gv(\xd)$ in \eref{gen_rate}, and any growth rate will give the same answer. Let's see. Under 
option (1) we have
\be
