...
dx^* &=& x^*(\mu^* dt + \sigma^* dW).
\eea
Assuming that both grow over time, meaning $\mu > \sigma^2/2$ and $\mu^* > {\sigma^*}^2/2$, the quantities $\D\x/\Dt$ and $\D\x^*/\Dt$ both diverge in the limit $\Dt\to\infty$. The growth is exponential, so linear additive changes will diverge over time. A 
...
\elabel{Du}
\eea
The monotonicity of $u(x)$ means that the events $\D\x>\D\x^*$ and 
...
with stationary (additive) increments.
\end{keypts}
