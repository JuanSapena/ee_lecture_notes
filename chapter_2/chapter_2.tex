...
model that's too restrictive. Imagine, instead of having the alphabet plus punctuation at your disposal you were only allowed to use sentences of five words, 
...
the criticism of utility-theory users who dislike the restriction to logarithmic utility functions.
...
plenty of situations where risk seeking (the opposite of risk aversion) is the sensible thing to 
do. 
...
situations. 1) my income is much more than what I need to live. I'm sure you know the 
...
stock market. 
2) you won't know about this, but here's another scenario. I don't have \$50,000 to put aside 
...
In situation 1) my wealth is roughly a multiplicative process and income is negligible. Not so 
in situation 2), where my income is so low that I save nothing, I can't pay my health 
insurance, can't buy a car to get a job in the next town, can't pay for university. I'm stuck at 
the bottom. But I know if even only \$1,000,000 fell into my lap my life would change 
dramatically. Not because I would have all the money I need but because my wealth would 
enter a different dynamical regime. 

In situation 1) I can relax. I'm happy with the status quo, and I don't need to do anything drastic to change it. In situation 2) that's not true. The status quo is terrible, it's perfectly sensible, for example, to play the lottery and hope for a big win, even if the expected return is negative. 

Let's analyse this situation from the perspective of utility theory, and then from the dynamic perspective. Utility theory would see me in situation 1) and conclude that my utility function is concave. My actions are optimal with respect to my utility function. Where that function comes from is unknown, nature vs. nurture. Similarly, in situation 2) my actions are optimal with respect to my utility function, which is convex -- I'm just the type of person who likes to take risks. A trained economist may also conclude that I'm a little stupid because I accept gambles with negative expected return.

From the dynamic perspective, the story is different. I'm required to supply a reasonable model of wealth dynamics. This will show that under situation 1) the growth-optimal thing to do is avoid big changes, just keep riding the stock market. In situation 2) the wealth dynamic reveals that I'm stuck at the bottom, and that the growth-optimal thing is to do whatever it takes to create change. It can't get much worse, so let's rock the boat. We will see an explicit mathematical example of this further down. 

Notice that the utility analysis focuses on the psychology of the decision maker. The dynamic perspective focuses on the situation of the decision maker.
...
We are interested in wealth -- more is better. We can position ourselves, make one choice or another, and then let time act. Choosing between two repeated gambles is thus a choice between two sequences of wealths, let's call them $\x(\t)$ and $\x^*(\t)$. If life were very simple we could just look at both of them at some moment in the future, $\t^*$ say, and choose the bigger one. But that's generally not possible because of noise -- both  $\x(\t^*)$ and $\x^*(\t^*)$ will be random numbers, and we don't know which one is bigger. 

So what do we do? Well, we just let time go to infinity, which should wash out any fluctuations and then choose the bigger one. So we consider
\bea
\lim_{\t\to\infty} \x(\t)\\
\text{and}\nonumber \\
\lim_{\t\to\infty} \x^*(\t).
\eea
But that's no good either because neither of these limits may exist. That's mathematics speak for ``they could both be infinite.'' In that case we can't compare because there's no obvious way to assign the ``greater than'' relationship between $\infty$ and $\infty$.

Ok, try again, it must be possible to make this work. What if we consider
