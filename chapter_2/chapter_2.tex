...
\be
\gat \equiv \lim_{\Dt\to\infty}  \left\{ \frac{\Du(\x)}{\Dt} \right\} =  \lim_{\Dt\to\infty} \{\gad(\gu)\}.
\elabel{barr}
\ee
If $\x(\t)$ satisfies certain conditions, to be discussed below, then the function 
$\gu(\x)$ can be chosen such that this limit exists. We shall see that $\gat(\gu(\x))$ is then the 
...
force ourselves to choose $\gu(\x)$ in a certain way. That certain way guarantees that
the correct form of growth rate is used. For example, if $\x(\t)$ is Brownian motion, $\gu(\x)$
will be linear, and if it's geometric Brownian motion, $\gu(\x)$ will be logarithmic. 
...
\be
\gat(\gu)=\frac{\ave{\Du}}{\Dt}.
\elabel{aver}
\ee
We could then make the identification of $\gu(\x)$ being the utility function, 
noting that our criterion is equivalent to maximizing the rate of change in
expected utility.
We note $\Du$ and hence $\gad(\gu)$ are random variables but $\ave{\Du}$ and $\gat(\gu)$ are not. 
...
In general, maximising $\gat$ does not yield the same decisions as 
the criterion espoused in \eref{max_Du}. This is only the case for a particular
function $\gu(\x)$ whose shape depends on the process $\x(\t)$, \ie on its dynamics. Our aim is to 
find these pairs of processes and functions. When using such $\gu(\x)$ as the utility 
...
using the utility function $\gu(\x)$ in purely dynamical terms: such behavior 
...
We ask what sort of dynamic $\gu$ must follow so that $\gat{\gu}=\ave{\gad(\gu)}$ or, 
put another way, so that $\gad(\gu)$ is an ergodic observable, 
...
\bea
\Du & \equiv & \gu(\tn+\Dt) - \gu(\tn) \\
& = & \sum_{m=1}^M \left[ \gu(\tn+m\dt) - \gu(\tn+(m-1)\dt) \right] \\
& = & \sum_{m=1}^M \du_m(\t),
\eea
where $\dt\equiv\Dt/M$ and $\du_m(\t)\equiv \gu(\tn+m\dt) - \gu(\tn+(m-1)\dt)$. From \eref{barr} we have
\begin{align}
\gat & = & \lim_{\Dt\to\infty} \left\{ \frac{1}{\Dt} \sum_{m=1}^M \du_m \right\} \elabel{barrSum}
\\
& = & \lim_{M\to\infty} \left\{ \frac{1}{M} \sum_{m=1}^M \frac{\du_m}{\dt} \right\} \elabel{barrSum2},
\end{align}
keeping $\dt$ fixed. From \eref{aver} we obtain
\be
\ave{\gad} = \lim_{\N\to\infty} \left\{ \frac{1}{\N} \sum_{n=1}^\N \frac{\Du_n}{\Dt} \right\}
\elabel{averSum}
\ee
...
consistency we then call $\du_m(\t)=\Du_m(\t)$. 
Expressions \eref{barrSum2} and \eref{averSum} are equivalent 
if the successive additive increments, 
$\Du_m(\t)$, are distributed identically to the $\Du_n$ in \eref{averSum}, 
...
Thus we have a condition on $\gu(\t)$ which suffices to make $\gat=\ave{\gad}$, 
namely that it be a stochastic process whose additive increments are 
stationary and independent. This means that $\gu(\t)$ is, in general, a L\'evy process. 
...
proved in \cite[Chapter~12]{Breiman1968}, $\gu(\t)$ must be a Brownian motion with drift,
\be
\gd\gu = a_u \gd\t + b_u \gd\gW,
\elabel{bm_u}
\ee
where $\gd\gW$ is the infinitesimal increment of the Wiener process.

By arguing backwards we can address concerns regarding the
existence of $\gat$. If $\gu$ follows the dynamics specified by 
\eref{bm_u}, then it is straightforward to show that the limit 
$\gat$ always exists and takes the value $a_u$. Consequently the 
...
$\gu(\x)$ is invertible, \ie provided that its inverse, $\x(\gu)$, exists, 
...
has failed to use the optimization of wealth growth over time to understand human behavior -- a criterion we consider extremely simple and natural. A  discussion of Menger's precise errors can be found in \cite[p.~7]{PetersGell-Mann2016}.

\subsubsection{Dynamic from a utility function}
We now illustrate the relationship between utility functions and
wealth dynamics with a few concrete examples. For the reasons discussed above we assume that
...
is equivalent to optimizing the time-average growth under the
...
$\gd\x$. In discrete time, two such pairs were investigated in \cite{PetersGell-Mann2016},
namely cases $1.$ and $2.$ below.

\begin{example}{1 Linear utility}
...
\begin{example}{2 Logarithmic utility} 
...
\begin{example}{3 Square-root (Cramer) utility}
