...
\section{Decisions\clabel{decision}}
...
\begin{definition}{Decision axiom}
...
We will begin in a setting where everything is known. In such a perfectly deterministic world, we will see that our
axiom leads to what's called ``discounting,'' a well-known behaviour of people facing inter-temporal choices.

In the next step, we will introduce noise -- randomness, ignorance, fluctuations. In that setting, 
just by holding on to our decision axiom we will again recover a well-known behaviour of people: maximization of expected utility functions.

\subsection{Decisions in a deterministic world}
\subsubsection{Different magnitudes}
I'm off to the bank to withdraw some money for you. I offer to give you either (1) \$20 when I 
get back or (2) \$50 when I get back. You tell me what you prefer.

Let's see what our decision axiom does for us. First of all, there's no uncertainty, I'm not lying to you, no one will rob me on my trip to the bank \etc
I haven't told you how long it will take me to get to the bank, so let's call that time interval $\Dt$.
...
our model humans will always go for option (2). That's good -- because I would have chosen option (2), and 
...
\subsubsection{Different magnitudes and times - discounting}
Let's make the decision a little harder: what if you offer me (1) \$1 in a month or (2) \$3 in two months?

Again, we will compute the two growth rates corresponding to options (1) and (2), and then choose the bigger one -- that's how we have been programmed to behave in the world our axiom is creating. But unlike in the previous case, the functional form of the growth rate will now be important. 
...
\be
\gm^{(1)} =\frac{\ln(\x+\$1)-\ln(\x)}{\frac{1}{12}\text{year}},
\ee
and
\be
\gm^{(2)} =\frac{\ln(\x+\$3)-\ln(\x)}{\frac{2}{12}\text{year}}.
\ee
Curiously, which is greater depends on my initial wealth, in our model world. If my wealth is \$100, then $\gm^{(1)}\approx 11.94\%$ p.a.., and 
$\gm^{(2)}\approx 23.76\%$ p.a., wherefore I will choose option 2.

But If my initial wealth is \$1, then $\gm^{(1)}\approx 831.8\%$ p.a. and $\gm^{(2)}\approx 659.2\%$ p.a., and I'd choose option 1.
...
In the economics literature, choosing in this way is called ``exponential discounting'' because it is mathematically equivalent to assigning to payments a value that decays exponentially with the waiting time, $\Dt$.

What about the additive growth rate? That would be the relevant growth rate to compute if, for instance, these payments to us are promised as a salary. You already know what you'd pick: \$1 per month, or \$3 per two months? The reason I know you know what you'd choose is that it doesn't depend on your initial wealth. Let's see, that's just working with the identity function $\gv(\x)=\x$ in \eref{gen_rate}.
\be
\gad^{(1)} =\frac{\$1}{\frac{1}{12}\text{year}},
\ee
and
\be
\gad^{(2)} =\frac{\$3}{\frac{2}{12}\text{year}}.
\ee
...
An interesting feature of optimizing additive growth rates is what's called ``preference reversal:'' let's keep our example as it is, except we now let time march forward, holding fixed the moments in time when the payments are to be made. Under these conditions, there comes a time, precisely after half a month, when option (2) is no longer preferred. This is most easily explained with a figure \fref{hyp_disc}.

\begin{figure}
\centering
\begin{picture}(200,80)(0,0)
 \put(-80,0){\includegraphics[width=.3\textwidth]{./chapter_decisions/figs/disc_1.JPG}}
 \put(20,110){\includegraphics[width=.3\textwidth, angle=-90]{./chapter_decisions/figs/disc_2.JPG}}
 \put(120,110){\includegraphics[width=.3\textwidth, angle=-90]{./chapter_decisions/figs/disc_3.JPG}}
\end{picture}
\caption{\small (A) At the beginning option (2) yields the highest additive growth rate; (B) after half a month, the options are equally good; (C) after 3/4 of a month, preference reversal has take place, and option (1) now yields the highest growth rate. As the first payment approaches, the associated growth rate diverges.}
\flabel{hyp_disc}
\end{figure}
...
Importantly, we need not resort to psychology to generate a host of behaviours, such as impatience of poorer individuals or preference reversal.

\subsection{Decisions in an uncertain world}
...
In the present section we will make our model world more realistic by introducing randomness. For our purposes, this randomness can simply be thought of as ignorance. Thus, we don't quite know what payoff we will receive -- we won't worry about why that is. We will leave the times at which payments are made deterministic. We could also relax this bit of determinism, of course, but we won't.

Instead we will generalise the concept of a payment -- generalise in the sense of randomise it. This introduces the concept of a gamble. We will also have to do something about our axiom: with random payouts, the growth rates on which we base our decisions will also be random. We get around that problem by interpreting ``growth rate'' in the axiom as ``time-average growth rate'' when there's randomness involved. The functional form of the rate that guarantees stability in time of rates  in the deterministic case will now guarantee ergodicity of rates, see below. This will allow us to use expectation values of rates when they are easier to compute.
