...
\item the probability of an outcome that is certain to happen, \ie one that includes all possible outcomes, must be one.
\ei
The latter is a normalisation condition which fixes the scale of the probabilities.

\boldhead{Discrete random variables}
When the set of outcomes is discrete, say $\left\{\z_\gj\right\}_{\gj=1}^M$, we assign a probability, $p_j$, to each outcome, $z_j$, such that
\be
...
\boldhead{Continuous random variables}
Most of the models we will study contain random variables whose possible outcomes form a continuous set. In this case, $\Z$ can take uncountably many values, to which we cannot assign non-zero probabilities while maintaining the normalisation condition. Instead, we must assign probabilities to intervals. We define a \PDFa, $\PDF_{\Z}(\z)$, such that the probability of $\Z$ being in the interval $(a,b)$ is given by
\be
\text{P}\left[a \leq \Z \leq b\right] = \int_a^b \PDF_{\Z}(\z)d\z.
\ee
...
%In this section we validate \fref{1_2} by computing analytically the average of $\x(\t)$ over infinitely many realizations, a quantity known as the expectation value.
The expectation value is usually introduced as the sum of all possible values, 
weighted by their probabilities.

\begin{defn}{Expectation value}
The expectation value of a random variable  $\z$ 
