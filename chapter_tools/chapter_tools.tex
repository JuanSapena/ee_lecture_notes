...
\be
\text{P}\left[\Z=\z\right] = \begin{cases}
...
\be
\text{P}\left[a \leq \Z \leq b\right] = \int_a^b \PDF_{\Z}(\z)\gd\z.
\ee
...
The \textit{expectation value}\footnote{Also known as expected value, mathematical expectation, first moment, and mean.} is a property of a random variable. Denoted by $\eval{\Z}$, it is formally defined as the integral of $\Z$ with respect to its probability measure. For our purposes, the following definitions are sufficient.
\begin{defn}{Expectation value}
If $\Z$ is a discrete random variable, its expectation value is the sum of all possible realisations, $\z_\gj$, weighted by their probabilities, $\p_\gj$:
\be
\eval{\Z}=\sum_\gj \p_\gj \z_\gj.
\elabel{exp_sum}
\ee 
If $\Z$ is a continuous random variable, its expectation value is the integral:
\be
\eval{\Z}=\int_{-\infty}^{+\infty} \z \PDF_{\Z}(\z) \gd\z.
\ee 
\end{defn}
...
The expectation value is a property of a mathematical object. We distinguish it from a conceptually different quantity, the \textit{ensemble average}, denoted by $\ave{Z}$. This is the many-realisations limit of the sample average we met when averaging over wealth trajectories of the coin game in \cref{Why}.  
\begin{defn}{Ensemble average}
The ensemble average of a random variable, $\Z$, is the many-realisations limit of its sample average,
\bea
\ave{\Z} &=& \lim_{\N\to\infty}\ave{\Z}_{\N}\\
&=& \lim_{\N\to\infty} \frac{1}{N} \sum_{\gi=1}^N \z_\gi,
\elabel{ens}
\eea
where $\z_\gi$ denotes the $i^\text{th}$ independent realisation of $\Z$.
\end{defn}

Note that, unlike the expectation value, the ensemble average of a random variable is not defined in terms of probabilities. It is the quantity to which average over realisations converge as the number of realisations grows. We followed this procedure when analysing the coin game: at each round of the gamble, we found sample averages of simulated wealth for increasingly large samples, from one to one million, plotted as functions of time in \fref{1_2}. As the sample size increased, we saw that the temporal fluctuations in the sample average decreased, which we took 

While   in Recall in the coin game that we because In this section we validate \fref{1_2} by computing analytically the average of $\x(\t)$ over infinitely many realizations, a quantity known as the expectation value.
We will define it as a limit instead, and then
...
\begin{proof}
Denote by  $\gn_\gj$ the number of times the value $\z_\gj$ is observed in a sample of $\N$ realisations of $\Z$. Call this number. 
