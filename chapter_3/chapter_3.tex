...
\be
J_u(t)=u(\ave{x(t)}) - \ave{u(x(t))}.
\elabel{J_u}
\ee
It's not easy to write a general expression for $J_u(t)$ in only $t$ and the model parameters $a_u$ and $b_u$, because it would involve the solution of the general wealth dynamic in \eref{dx}. However, we can still say something about how inequality evolves. Let's see what happens if we start with perfect equality, $x_i(0)=x_0$ with $x_0$ fixed, and let wealths evolve a little to $x(\D t)=x_0+\D x$, where $\D x$ is a random wealth increment generated by the wealth dynamic. The change in inequality would be
\be
\D J_u = u(\ave{x_0+\D x}) - \ave{u(x_0+\D x)},
\ee
since $u(\ave{x_0})=\ave{u(x_0)}=u(x_0)$.

We can now appeal to Jensen's inequality: if $u$ is a concave function, like the logarithm, then $\D J_u \geq 0$; while if $u$ is convex, like the curious exponential in \eref{test_dyn_u}, then $\D J_u \leq 0$. The only cases for which $\D J_u=0$ are if $\D x$ is non-random or if $u$ is linear. Thus, it is both randomness and the nonlinearity of the utility function (or ergodicity transformation) that creates a difference in growth rates and generates inequality.
...
\be
R \equiv \frac{\var[\ave{x(t)}_N]}{\ave{\ave{x(t)}_N}^2}.
\elabel{rel_var}
\ee
To be explicit, here the $\ave{\cdot}$ and $\text{var}(\cdot)$ operators, 
without $N$ as a subscript, 
refer to the mean and variance over all possible PEAs. The PEAs themselves, taken over finite samples of size $N$, are denoted $\ave{\cdot}_N$. \Eref{rel_var} simplifies to
\be
R = \frac{\frac{1}{N}\var[x(t)]}{\ave{x(t)}^2},
\ee
...
\elabel{diff2}
\ee
We substitute \eref{diff2} for $dy$ to get terms at orders $dW$, $dt$, $dW^2$, $dt^2$, and $dWdt$. The scaling of Brownian motion allows us to replace $dW^2$ by $dt$ and we ignore terms at $o\left(dt\right)$. This yields
\bea
d \left(y^2\right) = 2\sigma y^2\,dW - \left(2\tau-\sigma^2\right) y^2\,dt + 2\tau y\,dt.
%+ \tau^2\left(y - 1\right)^2\,dt^2 - \st{\tau\left(y - 1\right)\,\sigma y\,dW\,dt}
\eea
...
\be
\frac{d\ave{y^2}}{dt} = -\left(2\tau - \sigma^2\right) \ave{y^2} + 2\tau
\elabel{avediff2}
\ee
with solution
\be
\ave{y\left(t\right)^2} = \frac{2\tau}{2\tau - \sigma^2} + \left(\ave{y\left(0\right)^2} - \frac{2\tau}{2\tau - \sigma^2}\right) e^{-\left(2\tau - \sigma^2\right)t}.
\elabel{avediff3}
\ee

The variance $V\left(t\right)=\ave{y\left(t\right)^2}-1$ therefore follows
\be
