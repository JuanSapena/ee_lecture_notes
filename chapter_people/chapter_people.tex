...
\be
\gex = \frac{\D\ln\ave{x}}{\Dt}; \quad \gt = \frac{\D\ave{\ln x}}{\Dt}.
\ee
...
\bea
\hat{\gex} &=& \frac{\D\ln\ave{x}_N}{\Dt} \\
&=& \frac{1}{\Dt}\left[\ln\left(\frac{1}{N}\sum_{i=1}^N x_i(t+\Dt)\right) - \ln\left(\frac{1}{N}\sum_{i=1}^N x_i(t)\right)\right] \\
&\approx& \gex.
\elabel{ga_N}
\eea

For $\gt$, the situation is rather simpler since we are sampling from the normally distributed individual growth rate. The sampling distribution of the population average is
\be
\frac{\D\ave{\ln x}_N}{\Dt} \sim \N\left(\gt,\frac{\sigma^2}{N\Dt}\right).
\ee
...
\be
\sigma\sqrt{N\Dt} \gg 1,
\ee
which is easily satisfied for realistic parameters. Therefore, we can make the approximation
\bea
\hat{\gt} &=& \frac{\D\ave{\ln x}_N}{\Dt} \\
&=& \frac{1}{\Dt}\left(\frac{1}{N}\sum_{i=1}^N \ln x_i(t+\Dt) - \frac{1}{N}\sum_{i=1}^N \ln x_i(t)\right) \\
&\approx& \gt.
\elabel{gt_N}
\eea
...
It turns out that we need only to track the geometric average of the income distribution. To show this, we start from \eref{gt_N} and rearrange,
\bea
\hat{\gt} &=& \frac{1}{\Dt}\left(\frac{1}{N}\sum_{i=1}^N \ln x_i(t+\Dt) - \frac{1}{N}\sum_{i=1}^N \ln x_i(t)\right) \\
&=& \frac{1}{\Dt}\left[\ln \left( \prod_{i=1}^N x_i(t+\Dt) \right)^\frac{1}{N} - \ln \left( \prod_{i=1}^N x_i(t) \right)^\frac{1}{N} \right] \\
&=& \frac{\D\ln G_N}{\Dt},
\elabel{geometric}
\eea
...
\be
x(t) \exp(\hat{\gt}\Dt) = x(t) \exp \left( \sum_{i=1}^N \frac{g_i\Dt}{N} \right).
\ee
