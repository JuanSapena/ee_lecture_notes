...
their growth rate converges to $\gt$. While the two limiting behaviours $\N\to\infty$
...
which would be the $\N\to\infty$ limit of \eref{g_est}. Instead it is the limit of what we might call a diverging ``superverse,''
$\Su\to\infty$, if we want to use the language of ``universes.'' The superverse then consists of $\Su$ multiverses of $\N$ universes (trajectories) each.
\be
...
\seclabel{REM}
Since the publication of \cite{PetersKlein2013} we have learned, thanks to discussions with J.-P.~Bouchaud, that the key object in \eref{gest_4} -- the sum of lognormal random variates -- has been of interest to the mathematical physics community since the 1980s. The reason for this is Derrida's random energy model \cite{Derrida1980,Derrida1981}, which we will describe here. This section is somewhat ``physicsy'' and the key results are in \eref{gest_7} and \eref{gest_8}. It's safe to skip to them, if you like.

The model is defined as follows. Imagine a system whose $N= 2^K$ energy levels are iid normal random energies, $E_i$ (for $K$ spins). This is a very simple model of a disordered system, such as a spin glass, the idea being that the system is so complicated that we ``give up'' and model its energy levels as realisations of a random variable.
...
\be
Z = \sum_{\gi=1}^{N} \exp\left(\frac{E_\gi}{T}\right),
\elabel{Z}
\ee
where $T$ is the temperature and the random energy is
\be
E_\gi = \frac{J}{\sqrt{2}} K^{1/2} \xi_\gi.
\ee
$J$ is a constant parameter and $\xi_\gi\sim\mN(0,1)$ are iid standard normal variates. $E_\gi$ scales like $K^{1/2}$ to ensure an extensive thermodynamic limit \cite[p.~79]{Derrida1980}. 

The free energy is
\be
F = -T \ln Z = -T  \ln\left[\sum_{\gi=1}^{N} \exp\left(\frac{E_\gi}{T}\right)\right].
\elabel{F}
\ee
Derrida studied the expectation value of the free energy, $\ave{F}$, \ie the ensemble average over all possible realisations of the sum. He found a critical temperature, $T_c$, and expressions for $\ave{F}$ valid in the high ($T>T_c$) and low ($T<T_c$) temperature regimes of the model.

These expressions are potentially useful for us, because $\gest$ in \eref{gest_4} is a similar mathematical object to $F$: they are both sums of lognormal variates. Indeed, if we equate the constant parameters,
\be
\sigma = \frac{J}{\sqrt{2}},
\ee
...
\elabel{t_c}
\ee
with high temperature ($T>T_c$) corresponding to short time ($\t<\t_c$) and low temperature ($T<T_c$) corresponding to long time ($\t>\t_c$). Note that $t_c$ in \eref{t_c} scales identically with $\N$ and $\gsigma$ as the transition time in \eref{short_t}. So the simple, hand-waving arguments we made earlier about finite populations in \GBM were consistent with a careful mathematical analysis. That's always nice.
