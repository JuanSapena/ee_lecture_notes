...
%\underline{Extreme distributions:}
%\begin{itemize}
...
\PDF_\x(\x)=\frac{\N-1}{\N}\delta(\x-0)+\frac{1}{\N}\delta(\x-\N\ave{\x}_\N),
\ee
see \fref{wealth_dist_extreme}.

%\begin{figure}
%\begin{picture}(140,140)(20,30)
%\put(0,30)
%\includegraphics[width=.55\textwidth]{./chapter_people/figs/wealth_dist_equal.pdf}
%\put(190,30){\includegraphics[width=.55\textwidth]{./chapter_people/figs/wealth_dist_def_1.pdf}}
%%\put(0,160){A)}
%%\put(190,160){B)}
%%\put(18,133){$y_3(\t)$}
...
%%\put(130,215){\vector(1,-1){13}}
%%\end{picture}
%\caption{Maximally equal wealth distribution: everyone owns $\$100,000$.}
%\flabel{wealth_dist_equal}
%\end{SCfigure}
%
%\begin{SCfigure}
%%\begin{picture}(200,140)(20,30)
%%\put(0,30)
%\includegraphics[width=.55\textwidth]{./chapter_people/figs/wealth_dist_unequal.pdf}
%%\put(190,30){\includegraphics[width=.55\textwidth]{./chapter_people/figs/wealth_dist_def_1.pdf}}
%%\put(0,160){A)}
%%\put(190,160){B)}
%%\put(18,133){$y_3(\t)$}
...
%%\put(130,215){\vector(1,-1){13}}
%%\end{picture}
%\caption{Maximally unequal wealth distribution: one person owns $\$100$ billion, all others own nothing.}
%\flabel{wealth_dist_unequal}
%\end{SCfigure}
%\end{itemize}
%\FloatBarrier
...
\seclabel{sketch}
In \cite{PetersKlein2013} we studied the sample mean of \GBM, which we termed the \PEA. This is the average of $\N$ independent realisations of the random variable $\x(\t)$, \eref{sample}. Here we sketch out some simple arguments about how this object depends on $\N$ and $\t$.
...
\elabel{max_N}
\ee
These features are consistent with the numerical simulations shown in \fref{trajectories}.

Suppose we made a good guess in \eref{max_N}. We can say more about the maximum of the sample using extreme value theory. To leading order, the maximum of a sample of $\N$ iid standard normal variates scales like
\be
\max_{\gi=1}^\N \{\xi_\gi\} \sim \sqrt{2\ln\N}
\ee
...
\be
\ave{\x(\t)}_\N \sim \frac{1}{N} \exp\left[\left(\gmu-\frac{\gsigma^2}{2}\right)\t + \sqrt{2\ln\N} \gsigma\t^{1/2}\right]
\ee
...
In \cite{PetersKlein2013} we analysed \PEAs of \GBM analytically and numerically. Using \eref{GBM_sol} and assuming $\x_\gi(0)=1$ for all $\gi$, the \PEA can be written as
\be
...
%\elabel{g_est}
%\ee
%In \cite{PetersKlein2013} we proved that the $\t\to\infty$ limit for any finite $\N$ is the same as for the case $\N=1$, 
%\be
...
%\eea
%
%We didn't look in \cite{PetersKlein2013} at the expectation value of $\gest(\t,\N)$ for finite time and finite samples, but it's an interesting object that depends on $\N$ and $\t$ but is not stochastic. Note that this is not $\gest$ of the expectation value, which would be the $\N\to\infty$ limit of \eref{g_est}. Instead it is the limit of what we might call a diverging ``superverse,'' $\Su\to\infty$, if we want to use the language of ``universes.'' The superverse then consists of $\Su$ multiverses of $\N$ universes (trajectories) each.
%\be
%\ave{\gest(\t,\N)} = \frac{1}{\t}\ave{\ln(\ave{\x(\t)}_\N)} = f(\N,\t),
%\elabel{gest_3}
%\ee
%where, as previously, $\ave{\cdot}$ without subscript refers to the average over all possible samples (one sample now being a multiverse of $\N$ universes each), \ie we're computing $\lim_{\Su\to\infty}\ave{\cdot}_{\Su}$. The last two terms in \eref{gest_4} suggest an exponential relationship between ensemble size and time. The final term is a tricky stochastic object on which the properties of the expectation value in \eref{gest_3} will hinge. This term will be the focus of our attention: the sum of exponentials of normal random variates or, equivalently, log-normal variates.

\subsection{The random energy model}
\seclabel{REM}
Since the publication of \cite{PetersKlein2013} we have learned, thanks to discussions with J.-P.~Bouchaud, that $Z(\N,\t)$ in \eref{PEA_Z} -- the sum of lognormal random variates -- has been of interest to the mathematical physics community since the 1980s. The reason for this is Derrida's random energy model \cite{Derrida1980,Derrida1981}, which we will describe here.
...
Imagine a system of $K$ spins, whose $\N=2^K$ energy levels are iid normal random energies, $E\xi_\gi$, for some constant, $E$. This is a very simple model of a disordered system, such as a spin glass, the idea being that the system is so complicated that we ``give up'' and model its energy levels as realisations of a random variable.
%(We denote the number of spins by $K$ and the number of resulting energy levels by $\N$, while Derrida uses $\N$ for the number of spins).
...
\be
\mF = -T\ln\mZ.
% = -T  \ln\left[\sum_{\gi=1}^{\N} \exp\left(\frac{E_\gi}{T}\right)\right].
\elabel{F}
\ee
...
Note that $\tc$ in \eref{t_c} scales identically with $\N$ and $\gsigma$ as the transition time in \eref{short_t}. So the simple, hand-waving arguments we made earlier about finite populations in \GBM were consistent with a careful analysis from statistical physics. That's always nice.

\textbf{Simplify next parts...}

%Indeed, if we equate the constant parameters,
%\be
%\sigma = \frac{J}{\sqrt{2}},
%\ee
%and define a mapping between time in \GBM and temperature and spins in the random energy model,
%\be
%t^{1/2} = \frac{K^{1/2}}{T},
%\ee
%then we have
%\be
%\frac{E_\gi}{T} = \gsigma \t^{1/2} \xi_\gi
%\ee
%and we can write \eref{gest_4} as
%\be
%\gest = \gmu - \frac{\gsigma^2}{2} - \frac{\ln \N}{\t} - \frac{F}{T\t}.
%\elabel{gest_5}
%\ee
%
...
%
%Now we can make use of Derrida's results. He identifies a critical temperature,
%\be
%T_c \equiv \frac{J}{2\sqrt{\ln 2}},
%\elabel{T_c}
%\ee
%above and below which the expected free energy scales differently with $T$ and $K$. This maps to a critical time scale in \GBM,
%\be
%\tc = \frac{2\ln \N}{\gsigma^2},
%\elabel{t_c}
%\ee
%with high temperature ($T>T_c$) corresponding to short time ($\t<\tc$) and low temperature ($T<T_c$) corresponding to long time ($\t>\tc$).

%In \cite{Derrida1980}, $\ave{F}$ is computed for $T>T_c$ as
%\be
%\ave{F} = -KT\ln2 - \frac{K J^2}{4T},
%\elabel{F_2}
%\ee
%and for $T<T_c$ as
%\be
...
%\elabel{F_3}
%\ee
%Let's map these results to the short and long time regimes of \GBM.

\paragraph{\underline{Short time}}
\mbox{}

We look at the short-time behaviour first (high $T$, \eref{F_2}).
%The relevant computation of the entropy $S$ in \cite{Derrida1980} involves replacing the number of energy levels $\N(E)$ by its expectation value $\ave{\N(E)}$. This is justified because the standard deviation of this number is $\sqrt{\N}$ and relatively small when $\ave{\N(E)}>1$, which is the interesting regime in Derrida's case.
%
%For spin glasses, the expectation value of $F$ is interesting, supposedly, because the system may be self-averaging and can be thought of as an ensemble of many 
%smaller sub-systems that are essentially independent. The macroscopic behavior is then given by the expectation value.
Taking the expectation value of \eref{gest_5} and substituting for $\ave{F}$ from \eref{F_2}, we find
\be
\ave{\gest}^{\text{short}} = \gmu - \frac{\gsigma^2}{2} - \frac{\ln N}{t} + \frac{K\ln2}{\t}+ \frac{K J^2}{4T^2\t}.
\elabel{gest_6}
\ee
...
Next, we turn to the expression for the long-time regime (low $T$, \eref{F_3}). Taking expectation values of \eref{gest_5} and substituting from \eref{F_3}, we find
\bea
\ave{\gest}^{\text{long}} &=& \gmu - \frac{\gsigma^2}{2} - \frac{\ln{\N}}{\t} + \frac{KJ\sqrt{\ln 2}}{Tt} \nn\\
&=& \gmu - \frac{\gsigma^2}{2} - \frac{\ln{\N}}{\t} + \sqrt{\frac{2\gsigma^2\ln N}{t}}.
\elabel{gest_8}
\eea

\textbf{This result needs a clearer and shorter discussion...}

%which has the correct long-time asymptotic behavior. The form of the correction to the time-average growth rate
...
%{\bf \underline{Purple line:}} long-time behavior, based on the random energy model, \eref{gest_8}. {\bf \underline{Vertical line:}} Crossover between the regimes at $\tc=\frac{2\ln \N}{\gsigma^2}$, corresponding to $\beta_c=\frac{2(\ln 2)^{1/2}}{J}$.
