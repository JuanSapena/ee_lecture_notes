...
\seclabel{sketch}
In \cite{PetersKlein2013} we studied the sample mean of \GBM, \eref{sample}, which we termed the \PEA. Here we sketch out some simple arguments about how this object depends on $\N$ and $\t$.
...
These features are consistent with the numerical simulations shown in \fref{trajectories}, whose generation we recommend highly as an exercise. With our intuition thus corroborated, it is worth the effort to say something more about \eref{max_N} using extreme value theory. The maximum of a sample of $\N$ iid standard normal variates, $\xi_\gi\sim\mN(0,1)$ for $\gi=1\dots\N$, is distributed around a mean which scales like
\be
...
%\elabel{gest_2}
%\ee
%for all $\N\geq1$. Substituting \eref{PEA_2} in \eref{g_N} produces
%\bea
...
Imagine a system of $K$ spins, whose $\N=2^K$ energy levels are iid normal random energies, $E\xi_\gi$, with standard deviation, $E$. This is a very simple model of a disordered system, such as a spin glass, the idea being that the system is so complicated that we ``give up'' and model its energy levels as realisations of a random variable.
...
Armed with \eref{ln_Z_exp}, what can we say about the properties of the \PEA in \GBM? Let's write down growth rate experienced by a \PEA of size $\N$ after time $\t$: 
\bea
...
%\mbox{}
%
%We look at the short-time behaviour first (high $T$, \eref{F_2}). Taking the expectation value of \eref{gest_5} and substituting for $\ave{F}$ from \eref{F_2}, we find
%\be
