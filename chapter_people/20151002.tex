...
As we have seen, much of human decision-making is optimal under the assumptions 
...
prefer to look at it.  These two perspectives are informative -- from the set of 
trajectories we can judge what is likely to happen to an individual, \eg by following 
an individual trajectory for a long time, and from the random variable $x(t)$ at some 
fixed value of $t$ we can see how wealth is distributed in a population. 
...
\elabel{lognormal}
\ee
Remember that the expectation value $\ave{\ln x}=\ln x_0 + \left(\mu - \frac{\sigma^2}{2}\right)t$ is
not the logarithm of the expectation value, $\ave{\ln x}\neq \ln\ave{x}$. 
Taking expectations of both sides of \eref{GBM} yields
\be
d\ave{x}=\ave{x} \mu dt
\ee
with solution
...
\elabel{exp_x}
\ee
so that $\ln\ave{x}=\ln \ave{x_0} + \mu t$.
...
It is a well-established empirical observation that the upper tails of real wealth distributions 
tend to look more like a power law than a log-normal. Our trivial model does not
strictly reproduce this feature, but it is instructive to compare the lognormal distribution
to a power-law distribution. A power law PDF has the form 
\be
\PDF(x)\propto x^{-\alpha},
\ee
which implies that the logarithm of the PDF is proportional to the logarithm of $x$. Plotting
one against the other will yield a straight line, the slope being the exponent $\alpha$. 

Plotting any distribution on log-log scales results in some line whose local
slope may be interpreted as a local apparent exponent of the distribution. We ask
what the local apparent exponent of a log-normal distribution is near the 
...
that broaden real wealth distributions are missing from the null model. However, it is interesting
that the trivial model of GBM chimes with so many qualitative features of empirical observations. 

\subsubsection{Inequality measure from two growth rates}
\seclabel{Inequality_measure}
In a large population of $N$ individuals, $\D x \PDF(x)N$ is the approximate number of individuals 
who have wealth near $x$ at time $t$. In the case of GBM we have just seen how to 
...
Theil's second index of inequality, is the difference
...
%\ee
For instance, for large $N$ it is the average wealth of the population -- if everyone has the 
same wealth, everyone will own $\forall i, x_i=\ave{x}$. Whatever the distribution 
of wealth, the total wealth in a large population is approximately $N\ave{x}$. The growth 
rate of the expectation value, $\gave$, thus tells us how fast total population wealth and 
the average wealth grow with probability one in a large ensemble. The time-average 
...
This quantity turns out to be a well-known measure of inequality that economists identified
independently, without considering non-ergodicity and ensemble average and time average 
growth rates. We point out that the above definition may be used for dynamics other than
...
are called ``relative measures'', and $J$ is one of them. One of the conditions is that inequality measures
...
\be
J(t)=\ln \ave{x}-\ave{\ln x}=-\ave{\ln y},
\ee
which is known as the mean logarithmic deviation (MLD) or Theil's second index of inequality \cite{Theil1967}. 
Reading $\ave{\ln x}$ off \eref{lognormal} and using $\ave{x}$ of \eref{exp_x}, 
we find the dynamic of inequality for a population where individual wealth follows GBM
\be
J(t)=J(0)+\frac{\sigma^2}{2} t.
\ee
Inequality will increase indefinitely. 

This result implies an evolution towards wealth condensation. Wealth condensation 
...
would this operation affect the dynamic of the wealth of these two individuals?
 ...
 \bea
 \D x_i(t) & = & x_i(t) \left[\mu \Delta t + \sigma \sqrt{\Delta t}\,\xi_i\right], \elabel{discrete_nonc_grow} \\
 x_i(t+\D t) & = & x_i(t) + \D x_i(t), \elabel{discrete_nonc_coop}
 \eea
...
We imagine that the two previously non-cooperating entities, with resources $x_1(t)$ and $x_2(t)$, cooperate to produce two entities, whose resources we label $x^c_1(t)$ and $x^c_2(t)$ to distinguish them from the non-cooperating case. We envisage equal sharing of resources, $x^c_1=x^c_2$, and introduce a cooperation operator, $\oplus$, such that
 ...
 \be
 \D x^c_i(t) = x^c_i(t)\left[\mu\D t + \sigma\sqrt{\D t}\,\xi_i\right].
 ...
 \be
 x^c_i(t+\D t) = \frac{ x^c_1(t) + \D x^c_1(t) + x^c_2(t) + \D x^c_2(t)}{2}.
 ...
 \put(-20,0){\includegraphics[width=470pt]{./figs/blobs.pdf}}
 \put(0,145){$x^c_1(t)$}
 ...
 (x_1 \oplus x_2)(t) \left[1 + \left(\mu \Delta t + \sigma \sqrt{\Delta t} \, \frac{\xi_1 + \xi_2}{2}\right)\right].
...
whereas the cooperating unit grows at $g_{t}(x_1\oplus x_2)=\mu-\frac{\sigma^2}{4}$, 
and we have
\begin{equation}
g_{t}(x_1\oplus x_2)>g_{t}(x_i)
\end{equation}
for any non-zero noise amplitude. A larger time-average growth rate is the textbook definition of an 
evolutionary advantage. The effect is illustrated in \fref{cooperate} by direct simulation of
...
\put(-35,-135){\includegraphics[width=440pt]{./figs/cooperate.pdf}}
\end{picture}
...
A simple extension of the model is the inclusion of more cooperators. For $n$
cooperators, the spurious drift term is $-\frac{\sigma^2}{2n}$, so that the time-average
growth approaches expectation-value growth for large $n$. 
...
many cooperators, and adding further cooperators cannot improve on this.
 ...
 The approach to this upper bound as the number of cooperators increases favours 
 the formation of social structure. We may generalise to different drift 
 terms, $\mu_i$, and noise amplitudes, $\sigma_i$, for different individual entities. Again it 
is straightforward to see whether cooperation is beneficial in the long run for any
given entity. An entity whose drift term, $\mu_1$, is larger than a potential
cooperator's drift term, $\mu_2$, still benefits from cooperation if 
\be
\mu_1-\frac{\sigma_1^2}{2}<\frac{\mu_1+\mu_2}{2}-\frac{\sigma_1^2+\sigma_2^2}{8}.
\ee
Another generalisation is 
partial cooperation -- entities may share only
...
there is a critical tax rate -- below this tax rate inequality will increase indefinitely; 
above this tax rate inequality will stabilize.
This qualitative result applies both to income tax and to wealth tax.
...
\elabel{wsde}
\ee
To make the model a lot more tractable we consider the case $N \to \infty$, which replaces the finite-ensemble 
...
The solution of \eref{ODE2} is the second moment of the distribution of rescaled wealth as a function of time, namely
\be
...
that all moments of order $n>\frac{2\tau}{\sigma^2}+1$ diverge. Under a flat 
wealth tax as investigated here, the wealth distribution never fully stabilizes. In the language often
...
\begin{figure}
\bc
\includegraphics[width=0.8\textwidth]{./figs/var_wealth.pdf}
...
\begin{figure}
\bc
\includegraphics[width=0.8\textwidth]{./figs/hist_wealth.pdf}
...
\begin{figure}
\bc
\includegraphics[width=0.8\textwidth]{./figs/var_income.pdf}
...
It is striking that the finite-sample deviations from the infinite-ensemble result are larger in \fref{var_income} than in \fref{var_wealth}. This is due entirely to the simulated parameter values: \eref{wsde} and \eref{isde} can be made equivalent by choosing different parameters. 
...
\begin{figure}
\bc
\includegraphics[width=0.8\textwidth]{./figs/hist_income.pdf}
